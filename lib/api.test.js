...
(async () => {
  ...
  await test('fetches a conversation by explicit id outside a conversation page', async () => {
    let requestedUrl;
    const context = createContext({
      window: {
        location: {
      ...
      fetch: async (url) => {
        ...
        return {
          ok: true,
          status: 200,
          json: async () => ({ id: 'history-conversation' })
        };
      }
    });
    context.window.fetch = context.fetch;
    ...
    const result = await context.fetchConversationById('history-conversation');

    assert.strictEqual(
      requestedUrl,
      'https://chatgpt.com/backend-api/conversation/history-conversation'
    );
    assertJsonEqual(result, {
      success: true,
      data: {
  ...
  await test('rejects an empty explicit conversation id', async () => {
    const context = createContext();
    loadApi(context);

    const result = await context.fetchConversationById(null);
